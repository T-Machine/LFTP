	...
	public void run() {
		...
		try {
			while (nextSeq < data.size()) {
				if (nextSeq < base + N) {
					byte[] buffer = ByteConverter.toByte(data.get(nextSeq));
		...
		try {
			...
			byte[] buffer = ByteConverter.toByte(new Packet(0, 0, false, true, 0, null));
			DatagramPacket dp = new DatagramPacket(buffer, buffer.length, address, destPort);
			socket.send(dp);
		...
		public void run() {
			try {
				while (true) {
					...
					socket.receive(dp);
					Packet packet = ByteConverter.toObject(buffer);
	...
	private void timeOut() {
		...
		try {
			for (int i = base; i < nextSeq; ++i) {
				byte[] buffer = ByteConverter.toByte(data.get(i));
				DatagramPacket dp = new DatagramPacket(buffer, buffer.length, address, destPort);
